...
require get_template_directory() . '/admin/inc/study.php';
require get_template_directory() . '/admin/inc/news.php';
require get_template_directory() . '/admin/inc/event.php';
require get_template_directory() . '/admin/inc/special.php';
...
require get_template_directory() . '/admin/inc/native-ad.php';
...
/**
 * AVANT オプションTools デフォルト画像設定フィルター
 */
function avant_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		array(
			'media_filename' => 'avant-image_740x280.gif',
      ...
      'theme_option_keys' => array(
        ...
        'footer_links_banner_img3',
        'footer_links_banner_img4',
        'footer_links_banner_img5',
        'footer_links_banner_img6',
      )
		),
		array(
...
/**
 * AVANT オプションTools サンプルカテゴリー
 */
function avant_theme_options_tools_set_sample_categories( $dp_options ) {
...
/**
 * AVANT オプションTools サンプル記事
 */
function avant_theme_options_tools_set_sample_posts( $dp_options ) {

	// アイキャッチ メディアID
	$media_id['post'] = theme_options_tools_get_media_id( array(
		'media_filename' => 'avant-image_840x570.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/image_840x570.gif'
	), true );
  ...
  //$media_id['news'] = theme_options_tools_get_media_id( array(
	...
	$media_id['event'] = theme_options_tools_get_media_id( array(
		'media_filename' => 'avant-image_740x500.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/image_740x500.gif'
	), true );

	$media_id['special'] = theme_options_tools_get_media_id( array(
		'media_filename' => 'avant-image_900x615.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/image_900x615.gif'
	), true );
  ...
  for ( $i = 4; $i >= 1; $i-- ) {
    ...
    $sample_posts[] = array(
			'post_title' => __( 'Sample special', 'tcd-w' ) . $i,
			'post_content' => __( 'sample text sample text.', 'tcd-w' ),
			'post_name' => 'sample-special' . $i,
			'post_status' => 'publish',
			'post_type' => 'special',
      'thumbnail' => intval( $media_id['post'] )
		);
    if ( 4 !== $i ) {
		  $sample_posts[] = array(
		  	'post_title' => __( 'Sample event', 'tcd-w' ) . $i,
			  'post_content' => __( 'sample text sample text.', 'tcd-w' ),
		  	'post_name' => 'sample-event' . $i,
		  	'post_status' => 'publish',
		  	'post_type' => 'event',
        'thumbnail' => intval( $media_id['event'] ),
        'taxonomies' => array(
          'event_tag' => 'sample-event-tag'
        ),
        'metas' => array(
          'header_content' => 'on',
          'event_date' => date( 'Y-m-d', strtotime( 'next year' ) )
        )
      );
    }
	}
...
/**
 * AVANT オプションTools サンプルメニュー
 */
function avant_theme_options_tools_set_sample_menus( $dp_options ) {
...
/**
 * AVANT オプションTools サンプルウィジェット
 */
function avant_theme_options_tools_set_sample_widgets( $dp_options ) {
		...
		array(
			...
			'params' => array(
				...
				'banner_image1' => 	theme_options_tools_get_media_id( array(
					'media_filename' => 'avant-image_300x250.gif',
    ...
    // Styled post list3 (tcd ver)
		array(
			'class_name' => 'styled_post_list_widget3',
			'params' => array(
        'title' => __( 'Special post', 'tcd-w' ),
        'title1' => __( 'Title here. Title here.', 'tcd-w' ),
        'url1' => '#',
				'image1' => 	theme_options_tools_get_media_id( array(
					'media_filename' => 'avant-image_600x420.gif',
					'source_filepath' => get_template_directory() . '/assets/images/op_default/image_600x420.gif'
        ), true ),
        'target1' => 0,
        'title2' => '',
        'url2' => '',
 ...
 * AVANT テーマ初期化フィルター
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'avant_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'avant_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'avant_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'avant_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'avant_theme_options_tools_set_sample_widgets', 10, 1 );
