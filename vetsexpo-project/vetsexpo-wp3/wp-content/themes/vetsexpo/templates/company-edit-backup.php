<?php /* Template Name: CompanyEditOld */ ?>
<?php 
...
if(!empty($post_id)){
    ...
    $company_detail = get_post_meta( $company_detail, 'company_detail', true );
...
if( isset($_POST['action']) && $_POST['action'] == 'add_company' ) { // Check what the post type is here instead
    ...
    if($error == ''){
        ...
        //     } 
        // }
        print_r (get_post_meta($post_id));exit;
        foreach($uploads as $upload) {
            if(isset($_FILES[$upload]) && ($_FILES[$upload]['name'] != '')){
                $uploadedfile = $_FILES[$upload];
                $upload_overrides = array( 'test_form' => false );
                print_r (get_post_meat($post_id));exit;
...
<div class="imgupload-div">
    ...
    <div class="img-preview" data-spec="company_qrcode" style="width:300px;height:200px;background-image:url(<?php echo !empty($company_qrcode)?$company_qrcode:$noimage; ?>); background-repeat: no-repeat; background-size: cover; background-position: center;">
    </div>
    <input type="file" title="" class="img-input" name="company_qrcode" data-spec="company_qrcode"/>
